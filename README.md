...
All routes are prefixed with /api (e.g. POST /api/auth/login, GET /api/customers). A Postman collection covering every endpoint is at docs/postman_collection.json — import it, set the collection's baseUrl variable, run "Login" once, and the accessToken variable auto-populates for the rest of the requests.
